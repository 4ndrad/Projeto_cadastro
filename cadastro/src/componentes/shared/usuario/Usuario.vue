<template>
  <div class="admin">
    ...
    <ul v-for="cadastro of filtroCad" class="admin__usuario">
      ...
      <meu-botao
        rotulo="Deletar usuario"
        tipo="button"
        @botaoAtivado="removeCadastro(cadastro)"
        :confirmacao="true"
        estilo="perigo"
      />
      ...
      <li>
        ...
        <ul v-for="documento of cadastro.documentos" class="admin__usuario">
          ...
          />
          <router-link
            :to="{ name: 'alterarDocumento', params: { id: documento.idCad } }"
          >
            <meu-botao
              rotulo="Alterar "
              tipo="button"
      ...
      <li>
        ...
        <ul v-for="endereco of cadastro.enderecos" class="admin__usuario">
          ...
          />
          <router-link
            :to="{ name: 'alterarEndereco', params: { id: endereco.idEnd } }"
          >
            <meu-botao
              rotulo="Alterar "
              tipo="button"
    ...
    removeCadastro(cadastro) {
      ...
      this.$http.delete(`endereco/${cadastro.idCad}`).then(() => {
        this.$http.delete(`cadastro/${cadastro.idCad}`).then(
          ...
          (err) => {
            alert(
              "Não foi possível remover o usuario, recarregue a pagina e tente mais uma vez!"
            );
            console.log(err);
          }
        );
